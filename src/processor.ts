...
const archive = process.env.ARCHIVE_ENDPOINT; assert(!!archive);
const chain = process.env.CHAIN_ENDPOINT; assert(!!chain);
processor.setDataSource({ archive, chain });
processor.setTypesBundle('indexer/typesBundle.json')

processor.addEventHandler('btcRelay.StoreMainChainHeader', storeMainChainHeader);
...
processor.addExtrinsicHandler('relay.store_block_header', {
